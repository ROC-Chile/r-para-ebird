...
```{r}
writeRaster(trends_raster, filename = "ebird-tendencias_golo_2022.tif")
```
...
regiones <- ne_states(iso_a2 = "CL", returnclass = "sf") |>
...
trends_golo_sf <- st_join(trends_golo_sf, regiones, left = FALSE)
...
```{r}
trends_region_sf <- left_join(regiones, trends_region, by = "state")
...
```{r}
patos <- c("Pato jergón grande", "Pato jergón chico", "Pato gargantillo", "Pato capuchino", "Pato cuchara", "Pato colorado")

ebirdst_runs_chile |> 
  filter(nombre_comun %in% patos)

# obtengamos los codigos de especie para estas especies
codigo_patos <- ebirdst_runs_chile |> 
  filter(nombre_comun %in% patos) |>
  select("species_code") 
```
...
```{r}
ebirdst_download_trends(codigo_patos[[1]]$species_code)
```

Ahora podemos cargar las tendencias y calcular la media por celda.

```{r}
trends_patos_especies <- load_trends(codigo_patos[[1]]$species_code)
...
all_species <- trends_patos |> 
  filter(n_species == length(codigo_patos[[1]]$species_code)) |> 
...
ggplot(all_species) +
  ...
  labs(title = "Tendencias patos (2012-2022)",
