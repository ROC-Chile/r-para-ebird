...
The function `ebirdst_download_status()` downloads data for a single species given a species name (common name, scientific name, or species code). For example, to download the data for Green-backed Firecrown use:

```{r}
#| label: ebirdst-download-fake
#| eval: false
path <- ebirdst_download_status(species = "Green-backed Firecrown", path = "data/ebirdst-data")
path
```
...
ebirdst_download_status("Green-backed Firecrown", dry_run = TRUE)
```
...
#| eval: false
ebirdst_download_status("Green-backed Firecrown", pattern = "abundance", path = "data/ebirdst-data/")
```
...
path <- get_species_path("Green-backed Firecrown", path = "data/ebirdst-data", dataset = "status")
path
```
...
abd_median_3km <- load_raster(species = "grbfir1", path = "data/ebirdst-data/", product = "abundance", period = "weekly", metric = "median", resolution = "3km")
