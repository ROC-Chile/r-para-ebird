...
#| eval: false
ebirdst_download_status("grbfir1", pattern = "abundance_median_3km_2023", force = TRUE)
```
:::
...
#| eval: false
path <- ebirdst_download_status(species = "Green-backed Firecrown")
path
```
...
#| eval: false
ebirdst_download_status("Green-backed Firecrown", pattern = "abundance")
```
...
abd_median_3km <- load_raster(path, product = "abundance", resolution = "3km")
print(abd_median_hr)
```
...
abd_lower_27km <- load_raster(path, product = "abundance", metric = "lower", 
                            resolution = "27km")
abd_upper_27km <- load_raster(path, product = "abundance", metric = "upper", 
                            resolution = "27km")
```
...
abd_weekly <- load_raster(path, product = "abundance", resolution = "9km")
...
pop_grbfir1 <- get_species_path("grbfir1", path = "data/ebirdst-data/") %>% 
  load_raster(product = "percent-population", resolution = "9km")
...
for (i in seq_len(nrow(species_list))) {
  ...
  pop <- get_species_path(this_species$species_code, 
    ...
    load_raster("percent-population", period = "seasonal", resolution = "9km")
